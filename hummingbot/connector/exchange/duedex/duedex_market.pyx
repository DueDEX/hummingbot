...
from hummingbot.connector.exchange.duedex.duedex_api_order_book_data_source import DuedexAPIOrderBookDataSource
from hummingbot.connector.exchange.duedex.duedex_auth import DuedexAuth
from hummingbot.connector.exchange.duedex.duedex_in_flight_order import DuedexInFlightOrder
from hummingbot.connector.exchange.duedex.duedex_order_book_tracker import DuedexOrderBookTracker
from hummingbot.connector.exchange.duedex.duedex_utils import (
...
DUEDEX_ROOT_API = "https://api.duedex.com"


class DuedexAPIError(IOError):
    def __init__(self, error_payload: Dict[str, Any]):
        super().__init__(str(error_payload))
        self.error_payload = error_payload


cdef class DuedexMarketTransactionTracker(TransactionTracker):
    cdef:
        DuedexMarket _owner

    def __init__(self, owner: DuedexMarket):
        super().__init__()
        self._owner = owner
...
cdef class DuedexMarket(ExchangeBase):
    ...
    def logger(cls) -> HummingbotLogger:
        global market_logger
        if market_logger is None:
            market_logger = logging.getLogger(__name__)
        return market_logger

    def __init__(self,
                 duedex_api_key: str,
                 duedex_secret_key: str,
        ...
        self._order_book_tracker = DuedexOrderBookTracker(
        ...
        self._tx_tracker = DuedexMarketTransactionTracker(self)

        self._user_stream_event_listener_task = None
        self._user_stream_tracker = DuedexUserStreamTracker(duedex_auth=self._duedex_auth,
                                                            trading_pairs=trading_pairs)
    ...
    @property
    def order_book_tracker(self) -> DuedexOrderBookTracker:
    ...
    @property
    def in_flight_orders(self) -> Dict[str, DuedexInFlightOrder]:
    ...
    def restore_tracking_states(self, saved_states: Dict[str, Any]):
        self._in_flight_orders.update({
            key: DuedexInFlightOrder.from_json(value)
    ...
    async def get_active_exchange_markets(self) -> pd.DataFrame:
        return await DuedexAPIOrderBookDataSource.get_active_exchange_markets()
    ...
    async def check_network(self) -> NetworkStatus:
        try:
            await self._api_request(method="get", path_url="/v1/ticker/BTCUSDT")
    ...
    async def _api_request(self,
        ...
        if is_auth_required:
            headers = headers or {}
            headers.update(self._duedex_auth.get_rest_signature_dict(method, path_url, queryString, bodyString))
        ...
        async with response_coro as response:
            ...
            if data is None and parsed_response.get("code") != 0:
                self.logger().error(f"Error received from {url}. Response is {parsed_response}.")
                raise DuedexAPIError(parsed_response)
            # self.logger().error(f"Successfully received from {url}. Response is {parsed_response}.")
            return data

    async def _update_balances(self):
        cdef:
            str path_url = f"/v1/margin"
            list balances
        ...
        if len(balances) > 0:
            for balance_entry in balances:
                ...
                order_margin = Decimal(balance_entry["orderMargin"])
                position_margin = Decimal(balance_entry["positionMargin"])
                balance = available + order_margin + position_margin
                new_available_balances[asset_name] = available
    ...
    cdef object c_get_fee(self,
                          ...
                          object amount,
                          object price):
        # https://duedex.zendesk.com/hc/en-us/articles/360022357973-Perpetual-Swaps-Trading-Fees
        is_maker = order_type is OrderType.LIMIT_MAKER
        return estimate_fee("duedex", is_maker)
    ...
    async def _update_trading_rules(self):
        ...
        if current_tick > last_tick or len(self._trading_rules) < 1:
            exchange_info = await self._api_request("get", path_url="/v1/instrument")
    ...
    def _format_trading_rules(self, raw_trading_pair_info: List[Dict[str, Any]]) -> List[TradingRule]:
        ...
        for info in raw_trading_pair_info:
            try:
                if info["isInverse"]:
                    base_amount_precision = int(info["settlementCurrency"]["precision"])
                    quote_amount_precision = int(info["positionCurrency"]["precision"])
                else:
                    base_amount_precision = int(info["positionCurrency"]["precision"])
                    quote_amount_precision = int(info["settlementCurrency"]["precision"])
                trading_rules.append(
                    TradingRule(trading_pair=info["instrumentId"],
                                min_order_size=Decimal(info["lotSize"]),
                                max_order_size=Decimal(info["maxSize"]),
                                min_price_increment=Decimal(info['tickSize']),  # order price quantum
                                min_base_amount_increment=Decimal(info["lotSize"]),  # order size quantum, int
                                min_quote_amount_increment=Decimal(info["lotSize"]),
                                min_notional_size=Decimal(0)))
            except Exception:
                self.logger().error(f"Error parsing the trading pair rule {info}. Skipping.", exc_info=True)
        return trading_rules

    async def get_order_status(self, trading_pair: str, exchange_order_id: str) -> Dict[str, Any]:
        try:
            path_url = "/v1/order"
            params = {
                "instrument": convert_to_exchange_trading_pair(trading_pair),
                "orderId": exchange_order_id,
            }
            return await self._api_request("get", path_url=path_url, params=params, is_auth_required=True)
        except Exception as e:
            self.logger().network(
                f"Failed to get order {exchange_order_id} status: {str(e)}",
                exc_info=True,
                app_warning_msg=f"Failed to get order {exchange_order_id} status on Duedex. "
    ...
    async def _update_order_status(self):
        ...
        if current_tick > last_tick and len(self._in_flight_orders) > 0:
            ...
            for tracked_order in tracked_orders:
                ...
                try:
                    order_update = await self.get_order_status(tracked_order.trading_pair, exchange_order_id)
                except DuedexAPIError as e:
                    error_code = e.error_payload.get("code")
                    self.c_stop_tracking_order(tracked_order.client_order_id)
                    self.logger().info(f"The limit order {tracked_order.client_order_id} "
                                       f"has failed according to order status API. - {error_code}")
                ...
                if order_update is None:
                    self.logger().network(
                        ...
                        app_warning_msg=f"Could not fetch updates for the order {tracked_order.client_order_id}. "
                                        f"The order has either been filled or cancelled."
                    )
                    continue

                order_state = order_update["status"]
                # possible order states are "new", "partiallyFilled", "filled", "cancelled", "untriggered"
                if order_state not in ["new", "partiallyFilled", "filled", "cancelled", "untriggered"]:
                    self.logger().debug(f"Unrecognized order update response - {order_update}")

                # Calculate the newly executed amount for this update.
                tracked_order.last_state = order_state
                new_confirmed_amount = Decimal(order_update["filledSize"])
                execute_amount_diff = new_confirmed_amount - tracked_order.executed_amount_base

                if execute_amount_diff > s_decimal_0:
                    tracked_order.executed_amount_base = new_confirmed_amount
                    execute_price = Decimal(order_update["fillPrice"])
                    tracked_order.executed_amount_quote = Decimal(execute_amount_diff * execute_price)  # isInverse?
                    tracked_order.fee_paid = Decimal(order_update["accumulatedFees"])
        ...
        while True:
            try:
                ...
                self.logger().network("Unexpected error while fetching account updates.",
                                      exc_info=True,
                                      app_warning_msg="Could not fetch account updates from Duedex. "
        ...
        while True:
            try:
                ...
                self.logger().network("Unexpected error while fetching trading rules.",
                                      exc_info=True,
                                      app_warning_msg="Could not fetch new trading rules from Duedex. "
    ...
    async def _user_stream_event_listener(self):
        async for stream_message in self._iter_user_stream_queue():
            try:
                if "type" not in stream_message or stream_message["type"] not in ["snapshot", "update"]:
                    continue
                channel = stream_message["channel"]
                data = stream_message["data"]
                if channel == "margins":
                    for margin in data:
                        self._on_update_balance(margin)
                    continue
                elif channel == "orders":
                    for update_order in data:
                        self._on_order_update(update_order)
                else:
    ...
    async def execute_buy(self,
        ...
        try:
            ...
            self.logger().network(
                f"Error submitting buy {order_type_str} order to Duedex for "
                f"{decimal_amount} {trading_pair} "
                f"{decimal_price}.",
                exc_info=True,
                app_warning_msg=f"Failed to submit buy order to Duedex. Check API key and network connection."
            )
    ...
    async def execute_sell(self,
        ...
        try:
            ...
            order_type_str = order_type.name.lower()
            self.logger().network(
                f"Error submitting sell {order_type_str} order to Duedex for "
                f"{decimal_amount} {trading_pair} "
                f"{decimal_price}.",
                exc_info=True,
                app_warning_msg=f"Failed to submit sell order to Duedex. Check API key and network connection."
            )
            self.c_trigger_event(self.MARKET_ORDER_FAILURE_EVENT_TAG,
                                 MarketOrderFailureEvent(self._current_timestamp, order_id, order_type))
        ...
        try:
            ...
            if error_code == 10056:
                ...
                self.logger().network(
                    ...
                    exc_info=True,
                    app_warning_msg=f"Failed to cancel the order {order_id} on Duedex. "
                                    f"Check API key and network connection."
                )
        except Exception as e:
            self.logger().network(
    ...
    async def cancel_all(self, timeout_seconds: float) -> List[CancellationResult]:
        ...
        try:
            for order in open_orders:
                order_id = self.c_cancel(order.trading_pair, order.client_order_id)
                cancellation_results.append(CancellationResult(order_id, True))
        except Exception as e:
            self.logger().network(
                f"Failed to cancel all orders",
                exc_info=True,
                app_warning_msg=f"Failed to cancel all orders on Duedex. Check API key and network connection."
            )
    ...
    cdef c_start_tracking_order(self,
                                ...
                                object price,
                                object amount):
        self._in_flight_orders[client_order_id] = DuedexInFlightOrder(
    ...
    def _on_update_balance(self, data):
        asset_name = str(data["currency"]).upper()
        cached_data = self._account_cached_balances.get(asset_name, None)
        if not cached_data:
            cached_data = {
                "available": s_decimal_0,
                "orderMargin": s_decimal_0,
                "positionMargin": s_decimal_0,
            }
            self._account_cached_balances[asset_name] = cached_data
        if "available" in data:
            cached_data["available"] = Decimal(data["available"])
        if "orderMargin" in data:
            cached_data["orderMargin"] = Decimal(data["orderMargin"])
        if "positionMargin" in data:
            cached_data["positionMargin"] = Decimal(data["positionMargin"])
        balance = cached_data["available"] + cached_data["orderMargin"] + cached_data["positionMargin"]
